...
function initFloatingLabels() {
    const syncField = (input) => {
        const field = input.closest('.m-field');
        if (!field) return;
        field.classList.toggle('has-value', input.value.trim().length > 0);
    };

    document.querySelectorAll('.m-field input').forEach((input) => {
        syncField(input);
        input.addEventListener('input', () => syncField(input));
        input.addEventListener('change', () => syncField(input));
    });

    // Catch browser autofill after paint
    window.setTimeout(() => {
        document.querySelectorAll('.m-field input').forEach(syncField);
    }, 250);
}
